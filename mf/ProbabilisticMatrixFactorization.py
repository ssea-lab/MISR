...
class PMF(object):
    ...
    def save_UI(self,path):
        np.savetxt(path+'/U.factors',self.w_User)
        np.savetxt(path + '/I.factors', self.w_Item)
        print('save U_I,done!')

    def read_UI(self,path):
        w_User=np.loadtxt(path+'/U.factors')
        w_Item = np.loadtxt(path + '/I.factors')
